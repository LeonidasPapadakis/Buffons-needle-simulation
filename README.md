...
I created this project as part of an assessment for my Cardiff University Computer Science module *Computational Mathematics*.

This project simulates three variations of the Buffon's needle experiment using Matlab.

- `BuffonStandardPi.m`, `BuffonSquaresPi.m` and `BuffonSquaresRootTwo.m` run their respective simulation and 
return an estimate for pi / root two.

- `estimatePi.m`, `estimateRootTwo.m` and `throwSquares.m` are helper functions used by the simulations.

- `buffonsNeedle.mlapp` provides a GUI for running and visualising the experiment with an animation.
